...
`v0.2.0-beta.2` 已通过自动化、隔离 ZIP 和打包后进程链路，但还没有通过全新环境的
真实飞书录音 E2E。它作为 GitHub Pre-release 供学员自愿测试，不是稳定版；旧
`v0.1.0` 的真实证据不能挪作新架构证据。

## 安装前检查

课程验证基线是官方 `@larksuite/cli 1.0.81` 与 `lark-channel-bridge 0.6.4`。安装来源：

- 飞书 CLI：<https://github.com/larksuite/cli>
- Bridge：<https://github.com/zarazhangrui/feishu-claude-code-bridge>

不要安装 npm 上无 scope 的同名 `lark-cli` 包。每位学员必须扫码创建或绑定本人持有的
Bridge Profile，推荐统一命名为 `RecordingAgentCourse`；不得共享讲师配置或凭证。
...
Node.js 必须为 `>=20.12`，Codex CLI 必须已登录。版本号只是线索；缺少上述命令能力仍视为
未就绪。完整安装步骤见 `docs/MACOS.md`。
